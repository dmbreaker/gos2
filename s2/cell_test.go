...
func TestFaces(t *testing.T) {
	edgeCounts := map[Point]int{}
	vertexCounts := map[Point]int{}
	for face := 0; face < 6; face++ {
		id := CellIDFromFacePosLevel(face, 0, 0)
		cell := CellFromCellID(id)
		if cell.id != id {
			t.Errorf("%v != %v", cell.id, id)
		}
		if int(cell.face) != face {
			t.Errorf("%v != %v", cell.face, face)
		}
		if cell.level != 0 {
			t.Errorf("%v != 0", cell.level)
		}
		// Top-level faces have alternating orientations to get RHS
		// coordinates.
		if face&swapMask != int(cell.orientation) {
			t.Errorf("%v != %v", face&swapMask, cell.orientation)
		}
		for k := 0; k < 4; k++ {
			edgeCounts[cell.EdgeRaw(k)] += 1
			vertexCounts[cell.VertexRaw(k)] += 1
			got := cell.VertexRaw(k).Dot(cell.EdgeRaw(k).Vector)
			if got != 0.0 {
				t.Errorf("%v != 0.0", got)
			}

			got = cell.VertexRaw((k + 1) & 3).Dot(cell.EdgeRaw(k).Vector)
			if got != 0.0 {
				t.Errorf("%v != 0.0", got)
			}
			got = cell.VertexRaw(k).Cross(cell.VertexRaw((k + 1) & 3).Vector).Normalize().Dot(cell.Edge(k).Vector)
			if got-1.0 > 1e-14 {
				t.Errorf("%v != 1.0", got)
			}
		}
